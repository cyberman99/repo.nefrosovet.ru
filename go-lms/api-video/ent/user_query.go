...
func (uq *UserQuery) QuerySubscriber() *SubscriberQuery {
	query := &SubscriberQuery{config: uq.config}
...
func (uq *UserQuery) QueryUseraccount() *UserAccountQuery {
...
//  WithSubscriber tells the query-builder to eager-loads the nodes that are connected to
// the "subscriber" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithSubscriber(opts ...func(*SubscriberQuery)) *UserQuery {
	query := &SubscriberQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withSubscriber = query
	return uq
}
...
func (uq *UserQuery) WithUseraccount(opts ...func(*UserAccountQuery)) *UserQuery {
	query := &UserAccountQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUseraccount = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [2]bool{
			uq.withSubscriber != nil,
			uq.withUseraccount != nil,
		}
	)
	...
	if query := uq.withSubscriber; query != nil {
		...
		query.Where(predicate.Subscriber(func(s *sql.Selector) {
			s.Where(sql.InValues(user.SubscriberColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_subscriber
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_subscriber" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_subscriber" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Subscriber = n
		}
	}

	if query := uq.withUseraccount; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.UserAccount(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UseraccountColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_useraccount
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_useraccount" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_useraccount" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Useraccount = n
		}
	}
